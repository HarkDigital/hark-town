// Shared GLSL snippets. Concatenate into shader strings:
//   fragmentShader: NOISE + FBM + `void main(){ ... snoise(p) ... }`

/** hash12 / hash13 / hash33 helpers */
...
/** Ashima 3D simplex noise: float snoise(vec3) in [-1,1] */
...
/** fbm over snoise (requires NOISE). float fbm(vec3 p, int octaves) */
...
/** Rotation helpers: mat3 rotX/rotY/rotZ, vec3 rotateAxis(v, axis, angle) */
...
const vec3 SIGNAL = vec3(0.0, 1.0, 0.235);   // #00ff85 in linear space ≈ (0, 1, 0.235)
