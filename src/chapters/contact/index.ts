...
export default function create(): Chapter {
  ...
  const island = makeIsland({ radius: 5, seed: 7 })
  group.add(island)
  for (let i = 0; i < 6; i++) {
  ...
  return {
    ...
    group,
    init(ctx) {
      el('p', 'hud-eyebrow', 'Lighthouse', ctx.stage).style.cssText = 'position:absolute;left:var(--gutter);top:var(--safe-top)'
      title = rise(el('h2', 'hud-h2', undefined, ctx.stage), 'Say <em>hello.</em>')
      title.style.cssText = 'position:absolute;left:var(--gutter);top:calc(var(--safe-top) + 34px);max-width:60vw'
    },
    update(local, frame, ctx) {
      ctx.world.params.time = 0.9
