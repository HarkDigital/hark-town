...
export default function create(): Chapter {
  ...
  return {
    ...
    group,
    init(ctx) {
      el('p', 'hud-eyebrow', 'HQ', ctx.stage).style.cssText = 'position:absolute;left:var(--gutter);top:var(--safe-top)'
      title = rise(el('h2', 'hud-h2', undefined, ctx.stage), 'Make the internet <em>listen.</em>')
      title.style.cssText = 'position:absolute;left:var(--gutter);top:calc(var(--safe-top) + 34px);max-width:60vw'
    },
    update(local, frame, ctx) {
      ctx.world.params.time = 0.3
